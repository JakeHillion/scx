...
impl Scheduler {
    pub fn observe(&self, shutdown: &(Mutex<bool>, Condvar), timeout: Option<Duration>) -> Result<()> {
        ...
        while !*guard {
            if uei_exited!(&self.skel, uei) {
                return uei_report!(&self.skel, uei).and_then(|_| Err(anyhow::anyhow!("scheduler exited unexpectedly")));
            }
            if !timeout.is_none_or(|x| { Instant::now().duration_since(start_time) < x}) {
                break;
            }

            guard = cvar.wait_timeout(guard, Duration::from_millis(500)).unwrap().0;
        }
...
impl<'a> TryFrom<Builder<'a>> for Pin<Box<Scheduler>> {
    ...
    fn try_from(builder: Builder<'a>) -> Result<Pin<Box<Scheduler>>> {
        ...
        let open_object = unsafe {
            ...
            std::mem::transmute::<&mut MaybeUninit<OpenObject>, &'static mut MaybeUninit<OpenObject>>(open_object)
        };
        ...
        for tr in builder.traits {
            match tr {
                Trait::RandomDelays{ frequency, min_us, max_us } => {
                    open_skel.maps.rodata_data.random_delays_freq_frac32 = (frequency * 2_f64.powf(32_f64)) as u32;
