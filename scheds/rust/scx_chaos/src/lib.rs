...
impl Builder<'_> {
    fn load_skel(&self) -> Result<Pin<Rc<SkelWithObject>>> {
        ...
        for i in 1..freq_array.len() {
            freq_array[i] = freq_array[i].checked_add(freq_array[i - 1]).ok_or_else(|| {
                anyhow::anyhow!("frequencies overflowed! please ensure that frequencies sum to <=1."
                    " as these are floating point numbers, you may have to decrease by slightly"
