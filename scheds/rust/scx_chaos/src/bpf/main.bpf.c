...
{
	const u32 min_delay = random_delays_min_ns < random_delays_max_ns ?
				random_delays_min_ns : random_delays_max_ns;
	const u32 max_delay = random_delays_min_ns < random_delays_max_ns ?
...
void BPF_STRUCT_OPS(chaos_dispatch, s32 cpu, struct task_struct *prev)
{
	...
	bpf_for_each(scx_dsq, p, get_current_cpu_delay_dsq(), 0) {
		...
		if (taskc->run_after_time > now) {
			bpf_task_release(p);
			break; // this is the DSQ's key so we're done
		}

		async_p2dq_enqueue(&taskc->promise, p, taskc->promise.fifo.enq_flags);
		complete_p2dq_enqueue_move(&taskc->promise, BPF_FOR_EACH_ITER, p);
		bpf_task_release(p);
	}
...
void BPF_STRUCT_OPS(chaos_enqueue, struct task_struct *p __arg_trusted, u64 enq_flags)
{
	...
	async_p2dq_enqueue(&taskc->promise, p, enq_flags);

	if (taskc->next_trait != CHAOS_TRAIT_NONE &&
	    enqueue_chaotic(p, enq_flags, taskc))
		return;

	complete_p2dq_enqueue(&taskc->promise, p);
}
