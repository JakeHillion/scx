...
pub fn e2e_command(
    ...
    let config_content = fs::read_to_string(config_path)
        .context("Failed to read e2e-tests.toml")?;

    let config: E2eConfig = toml::from_str(&config_content)
        .context("Failed to parse e2e-tests.toml")?;
...
fn list_tests_json(config: &E2eConfig, kernel_filter: Option<&str>) -> Result<()> {
    ...
    for (scheduler_name, scheduler_config) in &config.scheduler {
        let default_kernels = scheduler_config.kernels.as_ref()
            .map(|k| k.clone())
            .unwrap_or_else(|| vec!["sched_ext/for-next".to_string()]);

        for test in &scheduler_config.tests {
            let test_kernels = test.kernels.as_ref()
    ...
    for (scheduler_name, scheduler_config) in &config.scheduler {
        ...
        for test in &scheduler_config.tests {
            ...
            for test_kernel in test_kernels {
                ...
                tests_run += 1;
                println!("Running test: {}::{} (kernel: {})", scheduler_name, test.name, test_kernel);
...
fn run_single_test(
    ...
    let mut scheduler_cmd = vec![binary_path.clone()];
    scheduler_cmd.extend(test.scheduler_args.iter().map(|s| s.clone()));

    // Build virtme-ng command
    let kernel_path = get_nix_kernel_path(kernel)
        .context("Failed to get Nix kernel path")?;

    // For now, just validate that we can build the binary and kernel, and show what we would run
    println!("  Binary path: {}", binary_path);
    println!("  Kernel path: {}", kernel_path);
    println!("  Scheduler command: {}", scheduler_cmd.join(" "));

    // Run test in virtme-ng using Nix shell (following the pattern from stress tests)
    // This runs the workload in background and then the scheduler with timeout
    let vm_input = format!(
        "{} & timeout {} {}",
        test.workload,
        test.timeout_sec,
    ...
    println!("  Running: nix shell ./.nix#virtme-ng --command vng {}", virtme_args.join(" "));

    let output = Command::new("nix")
        .args(["shell", "./.nix#virtme-ng", "--command", "vng"])
        .args(&virtme_args)
        .output()
        .context("Failed to run vng via nix shell")?;
...
fn get_nix_binary_path(package: &str) -> Result<String> {
    let output = Command::new("nix")
        .args(["build", "--no-link", "--print-out-paths", &format!("./.nix#{}", package)])
        .output()
...
fn get_nix_kernel_path(kernel: &str) -> Result<String> {
    ...
    let output = Command::new("nix")
        .args(["build", "--no-link", "--print-out-paths", &format!("./.nix#{}", kernel_package)])
        .output()
