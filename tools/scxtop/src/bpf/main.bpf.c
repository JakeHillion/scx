...
u64 trace_duration_ns = 1000000000; // TODO: fill me in
u64 trace_warmup_ns = 500000000; // TODO: fill me in
...
static __always_inline int start_trace_real(u64 trace_duration_ns, bool schedule_stop)
{
	...
	if (schedule_stop) {
		...
		if (!timerw)
			goto error_no_timer;
		if (bpf_timer_start(&timerw->timer, trace_duration_ns, 0))
			goto error_no_timer;
	}
...
{
	start_trace_real(trace_duration_ns + trace_warmup_ns, true /* schedule_stop */);
	return 0;
}
...
int BPF_URETPROBE(long_tail_tracker_exit)
{
	...
	return start_trace_real(trace_duration_ns, false /* schedule_stop */);
}
